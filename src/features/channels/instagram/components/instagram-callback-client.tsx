...
export function InstagramCallbackClient() {
	const searchParams = useSearchParams();
	const router = useRouter();
	const { handleCallback, isCallbackPending } = useInstagramCallbackMutation();

	const [inPopup, setInPopup] = useState(false);
	const [callbackResult, setCallbackResult] = useState<InstagramCallbackResult | null>(null);
	...
	useEffect(() => {
		...
		if (window.opener) {
			window.opener.postMessage(
				{ type: 'INSTAGRAM_OAUTH_CODE', code, state },
		...
		<div className="flex min-h-[60vh] items-center justify-center">
			{isCallbackPending && (
				<div className="flex flex-col items-center gap-3 text-center">
					<div className="border-primary h-8 w-8 animate-spin rounded-full border-2 border-t-transparent" />
					<p className="text-muted-foreground text-sm">Connecting your Instagram account...</p>
				</div>
			)}
			{callbackResult && (
				<InstagramAccountSelectionDialog
					...
					accounts={callbackResult.accounts}
